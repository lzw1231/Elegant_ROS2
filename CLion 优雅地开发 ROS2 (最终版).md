...
- **Working directory**：填写工作空间根目录，推荐使用通用宏，适配所有设备：
        `$ProjectFileDir$`
        也可填写本地绝对路径：`/home/lzw/Projects/clion/Elegant_ROS2`

#### 10\.2\.4 加载环境变量文件（关键步骤）

点击 Path to ".env" files 右侧文件夹图标 → 选择 `Load from file` → 选中第一步生成的根目录 `.env` 文件。
